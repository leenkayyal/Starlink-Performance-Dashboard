...
# ======================
st.set_page_config(
    page_title="Starlink Performance Forecast Dashboard V2",
    layout="wide"
)

st.title("Starlink Performance Forecast Dashboard")
st.caption("Monitoring, forecasting, health scoring, and alerting tool for university use")
...
TARGET_LATENCY = "ping_avg_rtt_ms"
...
def build_metric_dataset(clean_df, target_col):
    """
    Build a forecasting dataset for any metric such as:
    - download_mbps
    - upload_mbps
    """
    df = clean_df.copy()
...
def train_model(df, model_name):
    ...
    if model_name == "Naive Baseline":
        ...
        if not lag_candidates:
            raise ValueError("No lag_1 column found for naive baseline.")
...
with tab4:
    ...
    st.write(
        f"""
        The current forecast-based health status is **{health_label}** with a score of **{health_score}/100**.
...
with tab5:
    ...
    st.write(
        """
        The naive baseline is used as a benchmark.
        Linear Regression is the main forecasting model because it provided the best balance of accuracy, stability, and interpretability.
        More complex models were included as advanced comparisons.
        """
    )

st.markdown("---")
st.write("Dashboard V2 focuses on Starlink performance monitoring and forecasting for practical university use.")