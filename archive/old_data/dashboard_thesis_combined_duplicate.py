...
# FILE PATHS
# ======================
# The dashboard now supports TWO validated research periods:
# Period 1: the original March 7–28, 2026 Starlink experiment.
# Period 2: the additional validated Starlink forecast dataset used for retraining.
# Live/raw logging is still routed separately and never mixed into the research model.

# Historical charts and KPI cards use the clean measurement file.
# If you later create Cleaned/starlink_clean_combined.csv, the loader below will use it automatically.
THESIS_CLEAN_FILE = "Cleaned/starlink_clean_FIXED.csv"
THESIS_CLEAN_COMBINED_FILE = "Cleaned/starlink_clean_combined.csv"

# Forecasting/model evaluation uses the feature-engineered combined dataset.
# This file is created from:
# - Cleaned/starlink_forecast_v2.csv
# - Cleaned/starlink_2_forecast.csv
THESIS_FORECAST_FILE = "Cleaned/starlink_forecast_combined.csv"
FORECAST_PERIOD_1_FILE = "Cleaned/starlink_forecast_v2.csv"
FORECAST_PERIOD_2_FILE = "Cleaned/starlink_2_forecast.csv"

# Fallback names for local development, in case the two CSVs are placed beside the app.
FORECAST_PERIOD_1_FALLBACK = "starlink_forecast_v2.csv"
FORECAST_PERIOD_2_FALLBACK = "starlink_2_forecast.csv"
...
# Research period boundaries.
# Period 1 is known from the original validated experiment.
PERIOD_1_START = pd.Timestamp("2026-03-07 01:00:00")
PERIOD_1_END   = pd.Timestamp("2026-03-28 11:00:00")

# Period 2 is the additional validated dataset.
# Update these two dates if you know the exact collection window.
PERIOD_2_START = pd.Timestamp("2026-03-29 00:00:00")
PERIOD_2_END   = pd.Timestamp("2026-04-30 23:59:59")

# Backward-compatible names used by older parts of the dashboard.
# These now represent the full validated research window.
THESIS_START = PERIOD_1_START
THESIS_END   = PERIOD_2_END

RESEARCH_DATASET_LABEL = "Combined validated Starlink datasets: Period 1 March 7–28, 2026 + Period 2 additional forecast dataset"
FORECAST_DATASET_LABEL = "Cleaned/starlink_forecast_combined.csv"
...
# ======================
# DATASET MERGE / VALIDATION HELPERS
# ======================
def first_existing_path(paths):
    """Return the first existing file path from a list, or None if none exist."""
    for path in paths:
...
def ensure_combined_forecast_file():
    """
    Ensures the combined feature-engineered forecast dataset exists.

    If Cleaned/starlink_forecast_combined.csv is already present, it is used.
    If it is missing, the dashboard tries to merge the two period datasets automatically.
    This keeps deployment simple: either commit the combined file, or commit the two source files.
    """
    os.makedirs(os.path.dirname(THESIS_FORECAST_FILE), exist_ok=True)

    if os.path.exists(THESIS_FORECAST_FILE):
        return THESIS_FORECAST_FILE, "combined_existing"

    period_1_path = first_existing_path([FORECAST_PERIOD_1_FILE, FORECAST_PERIOD_1_FALLBACK])
    period_2_path = first_existing_path([FORECAST_PERIOD_2_FILE, FORECAST_PERIOD_2_FALLBACK])

    if period_1_path is None or period_2_path is None:
        missing = []
        if period_1_path is None:
            missing.append(f"{FORECAST_PERIOD_1_FILE} or {FORECAST_PERIOD_1_FALLBACK}")
        if period_2_path is None:
            missing.append(f"{FORECAST_PERIOD_2_FILE} or {FORECAST_PERIOD_2_FALLBACK}")
        raise FileNotFoundError(
            "Missing forecast dataset(s). Commit Cleaned/starlink_forecast_combined.csv "
            "or provide both source files: " + ", ".join(missing)
        )

    df1 = pd.read_csv(period_1_path)
    df2 = pd.read_csv(period_2_path)

    common_cols = [c for c in df1.columns if c in set(df2.columns)]
    if "target" not in common_cols:
        raise ValueError("Cannot merge forecast datasets because the shared 'target' column is missing.")

    combined = pd.concat([df1[common_cols], df2[common_cols]], ignore_index=True)

    # Convert feature-engineered values to numeric where possible and remove unusable rows.
    for col in combined.columns:
        combined[col] = pd.to_numeric(combined[col], errors="coerce")
    combined = combined.dropna().drop_duplicates().reset_index(drop=True)

    if len(combined) < 10:
        raise ValueError("Combined forecast dataset is too small after cleaning. Check the source CSV files.")

    combined.to_csv(THESIS_FORECAST_FILE, index=False)
    return THESIS_FORECAST_FILE, "combined_created"


def clean_forecast_dataframe(df):
    """Make sure the forecast dataset is numeric and model-ready."""
    df = df.copy()
    ...
    if "target" not in df.columns:
        raise ValueError("Forecast dataset must contain a 'target' column.")
    return df
...
def load_thesis_clean():
    """
    Loads validated clean Starlink measurements for historical charts and KPI cards.

    Priority:
    1. Cleaned/starlink_clean_combined.csv, if you create it later.
    2. Cleaned/starlink_clean_FIXED.csv, the original March 7–28 clean dataset.

    Live/raw measurements are never merged here.
    """
    clean_path = THESIS_CLEAN_COMBINED_FILE if os.path.exists(THESIS_CLEAN_COMBINED_FILE) else THESIS_CLEAN_FILE

    if not os.path.exists(clean_path):
        st.error(
            f"Missing clean measurement file: {clean_path}. "
            "The dashboard needs a clean measurement file for KPI cards and historical charts."
        )
        st.stop()
    ...
    if "timestamp" not in df.columns:
        st.error(f"The clean measurement file {clean_path} must contain a timestamp column.")
        st.stop()

    df["timestamp"] = pd.to_datetime(df["timestamp"], format="mixed")

    # Keep only the validated research window when timestamps are available.
    df = df[(df["timestamp"] >= THESIS_START) & (df["timestamp"] <= THESIS_END)]

    if len(df) == 0:
        st.error(
            "No clean rows remain after applying the validated research date filter. "
            "Check PERIOD_1/PERIOD_2 date boundaries or the clean CSV timestamps."
        )
        st.stop()
...
def load_thesis_forecast():
    """Loads the combined feature-engineered forecast dataset used for model training/evaluation."""
    forecast_path, merge_status = ensure_combined_forecast_file()
    df = pd.read_csv(forecast_path)
    return clean_forecast_dataframe(df), merge_status
...
with st.sidebar:
    ...
    st.markdown("Muscat, Oman · Combined datasets")
...
thesis_forecast, forecast_merge_status = load_thesis_forecast()
...
# Train on the combined validated forecast dataset
...
if show_live and latest_live_row is not None:
    ...
    kpi_note    = "📊 Showing last observation from validated clean research dataset"
...
">
    ...
    <div>
        ...
        <span style="color:#64748b; font-size:0.82rem; margin-left:0.8rem;">
            ...
            March 7–28, 2026 plus the additional validated forecast dataset. Live logging is routed separately
...
st.markdown(f"""
<div style="
    background:rgba(16,185,129,0.06); border:1px solid #10b98133;
    border-radius:8px; padding:0.5rem 1rem; margin-bottom:0.8rem;
    font-size:0.82rem; color:#94a3b8;
">🧠 Forecast model dataset: <b>{FORECAST_DATASET_LABEL}</b> · Rows: <b>{len(thesis_forecast):,}</b> · Status: <b>{forecast_merge_status.replace('_', ' ')}</b></div>
""", unsafe_allow_html=True)
    ...
    with left:
        st.plotly_chart(
            make_time_chart(thesis_clean_df, "timestamp", "ping_avg_rtt_ms",
                            "Latency Over Time (Validated Research Dataset)", "Latency (ms)",
                            color="#8b5cf6"),
...
# ---------- TAB 3: FORECASTING ----------
with tab3:
    st.markdown('<div class="section-title">Forecasting — Model trained on combined Starlink datasets</div>',
                unsafe_allow_html=True)
    st.plotly_chart(
...
with tab5:
    st.markdown('<div class="section-title">Model Evaluation — Combined Forecast Dataset</div>',
                unsafe_allow_html=True)
